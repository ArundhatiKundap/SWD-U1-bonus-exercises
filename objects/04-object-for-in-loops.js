...
/*
	Let's talk about the syntax of a for...in loop. It's actually not that different overall from a for loop, except you're not declaring 
	an iterator (i) and telling it stop/start/increment values. 
	Instead, you declare a generic variable to stand in for each key name. 
	This is great, because you may not even know what the key names are, but you can loop through anyway.
...
// What if we just need to find the key name for a particular item we think might be in there somewhere? 
//Let's write a function to look for some food, and return the key.
...
//values which belong to food items that are sides.No need to write this one as a function, just do the loop.Hint 1: you need to check the key names, not the values! Hint 2: Even though they don't look like it in the object, keys are stored as strings.
...
//but we handled that by taking the length of each array, as pantry[packageType] is the correct way to refer to the value of each property.

console.log(""); // skip a line in console

// TODO: Write a function that takes in an index number, prints "Items at index (x):"
//and then prints the element at that index of each array as you loop through the object.
//Use the escape character \t to indent each item for that index.If the array doesn't have anything at that index,
//it should be skipped without throwing an error. (Note: you don't need nested loops for this one.)

function printIndex(pantry) {
	for (let pantryKey in pantry) {
		for (let i = 0; i < pantry[pantryKey].length; i++) {
			console.log(`${i}\t`,pantry[pantryKey][i]);
		}
	}
}
//printIndex(pantry);

function takePrintedIndex(indexval) {
	for (let key in pantry) {
		if (pantry[key][indexval]) {
			console.log(`${indexval}\t`, pantry[key][indexval]);
		}
		else {
...
function displayItems(packageType) {
	console.log(`Items that are ${packageType.toUpperCase()}:`);
	for (let i = 0; i < pantry[packageType].length; i++) {
		console.log(`- ${pantry[packageType][i]}`);
	}
}
